...
export const POST: RequestHandler = async ({ request, locals: { supabase, supabaseService } }) => {
	try {
		...
		while (true) {
			...
			if (!treeNode) throw { status: 400, message: 'Invalid UUID' };
			...
			if (type === 'Default')
				postPromises.push(supabaseService.from('Nodes').delete().eq('uuid', uuid));
			else if (type === 'Thread')
				postPromises.push(supabaseService.from('Threads').delete().eq('uuid', uuid));
