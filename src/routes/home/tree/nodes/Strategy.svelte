<script lang="ts">
	...
	async function pushToStrategySuggestions(d: any): Promise<void> {
		if (posting) await waitForServer();
		posting = true;
		try {
			const response = await fetch('/home/tree/actions/push_strategy_suggestion', {
				method: 'POST',
				headers: {
	...
	async function pushToStrategyEdit(d: any): Promise<void> {
		...
		posting = true;
		try {
			const response = await fetch('/home/tree/actions/push_strategy_edit', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
